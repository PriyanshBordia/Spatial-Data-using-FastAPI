...
def check_country(db: Session, id=None, code=None) -> bool:
    try:
        if id is not None:
            country = (
                db.query(models.Country).filter(models.Country.id == id).one_or_none()
            )
        elif code is not None:
            country = (
                db.query(models.Country)
                .filter(models.Country.iso_a3 == code)
                .one_or_none()
            )
...
def get_country_by_name(db: Session, name: str) -> dict:
    ...
    try:
        country = (
            ...
            .filter(models.Country.admin == name)
            .one_or_none()
        )
        if country is not None:
            return success_response(data=[format(country)])
...
def get_country_by_name_contains(db: Session, name: str) -> dict:
    ...
    try:
        countries = (
            db.query(models.Country)
            .filter(models.Country.admin.contains(name))
            .with_entities(
    ...
    try:
        if not check_country(db, code=country.iso_a3):
            ...
            return success_response(
                data=[country], message="Country inserted successfully."
            )
        else:
...
def update_country(db: Session, id: int, country: schemas.CountryCreate) -> dict:
    ...
    try:
        if check_country(db, id=id):
            ...
            db.commit()
            return success_response(
                data=[country], message="Country updated successfully."
            )
        else:
...
def delete_country(db: Session, id: int) -> dict:
    ...
    try:
        ...
        if country is not None:
            ...
            db.commit()
            return success_response(
                data=[country], message="Country deleted successfully."
            )
        else:
            return error_response(error=[(f"Country with id: {id} does not exists.!")])
    except Exception as e:
