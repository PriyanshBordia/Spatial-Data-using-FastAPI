...
def get_country_by_id(db: Session, id: int) -> dict:
    ...
    try:
        country = (db.query(
            models.Country.id,
            models.Country.admin,
            models.Country.iso_a3,
            models.Country.geom.ST_AsGeoJSON(),
        ).filter(models.Country.id == id).one_or_none())
...
def get_country_by_code(db: Session, code: str) -> dict:
    ...
    try:
        country = (db.query(
            ...
            models.Country.iso_a3,
            models.Country.geom.ST_AsGeoJSON(),
        ).filter(models.Country.iso_a3 == code.upper()).one_or_none())
        if country is not None:
            return success_response(data=[format(country)])
        else:
            return error_response(
...
def get_country_by_name(db: Session, name: str) -> dict:
    ...
    try:
        country = (db.query(
            ...
            models.Country.iso_a3,
            models.Country.geom.ST_AsGeoJSON(),
        ).filter(models.Country.admin == name).one_or_none())
        if country is not None:
            return success_response(data=[format(country)])
        else:
            return error_response(
...
def get_country_by_name_contains(db: Session, name: str) -> dict:
    ...
    try:
        countries = (db.query(models.Country).filter(
            models.Country.admin.contains(name)).with_entities(
...
def get_neighboring_countries(db: Session, id: int) -> dict:
    ...
    try:
        ...
        if country is not None:
            ...
            ).with_entities(models.Country.id, models.Country.admin,
                            models.Country.iso_a3).all())
            if len(neighbors) > 0:
